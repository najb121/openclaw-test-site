...
export default function WhyChooseUs() {
  ...
  return (
    <section id="why-us" className="py-20 md:py-32 relative bg-slate-50 dark:bg-transparent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {reasons.map(({ key, icon: Icon, gradient }) => (
            <div
              key={key}
              className="relative group p-6 rounded-3xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-green-500/30 transition-all duration-300 hover:-translate-y-2 shadow-sm dark:shadow-none"
            >
              {/* Background gradient on hover */}
              <div className={`absolute inset-0 bg-gradient-to-br ${gradient} opacity-0 group-hover:opacity-5 dark:group-hover:opacity-10 rounded-3xl transition-opacity duration-300`} />
