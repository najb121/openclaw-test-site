        ...
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${dir === 'rtl' ? 'lg:grid-flow-dense' : ''}`}>
              ...
              >
                <div className="glass-card rounded-xl px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-900 dark:text-white">Licensed Coach</span>
                  </div>
                </div>
              </motion.div>
            </div>
          ...
          <div className={dir === 'rtl' ? 'lg:col-start-1 lg:row-start-1' : ''}>
            ...
            <ScrollReveal delay={0.2}>
              <p className="text-body-lg text-slate-600 dark:text-gray-400 mb-10">
                {t.about.description}
              </p>
            </ScrollReveal>
            ...
            <StaggerContainer className="space-y-4" staggerDelay={0.15}>
              {features.map((feature, idx) => (
                <StaggerItem key={idx}>
                  <motion.div
                    className={`group flex gap-4 p-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all duration-300 ${dir === 'rtl' ? 'flex-row-reverse' : ''}`}
                  ...
                  >
                    ...
                    <div className={dir === 'rtl' ? 'text-end' : ''}>
                      <h3 className="font-semibold text-slate-900 dark:text-white mb-1 text-lg">{feature.title}</h3>
